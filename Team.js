var Team = function(isAttacker, isEvolving, teamsize) {
  ...
  for (var i = 0; i < teamsize; i++) {
    this.player.push(new Player(isAttacker));
  }
  ...
  this.evolve = function(mutationRate) {
    ...
    if (this.isEvolving) {
      ...
      for (var i = 0; i < teamsize; i++) {
        ...
        newPlayers[i] = new Player(isAttacker);
  ...
  this.nomFitness = function() {
    if (isAttacker) {
      ...
      var avg = (outputsum - teamsize) / teamsize;
      avgOpponentFitnessP.html("Avg Opponent Lifespan: " + avg);
      dia.addData(avg);
    }
  }
